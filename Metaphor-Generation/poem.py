...
metaphor_model_path = "./model/model" # metaphor detection model file
threshold = 0.95

main(threshold, input_file, output_dir, metaphor_model_path, device)

os.system('python comet-commonsense/scripts/generate/generate_conceptnet_arbitrary_trans.py --model_file comet-commonsense/pretrained_models/conceptnet_pretrained_model.pickle --input_file ' + os.path.join(output_dir, "mlm_ans.json") + ' --output_file ' + os.path.join(output_dir, "poem_0.95_3.json") + ' --device ' + device + ' --sampling_algorithm beam-5')
